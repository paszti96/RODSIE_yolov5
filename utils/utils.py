...
def build_targets(p, targets, model):
    ...
    det = model.module.model[-14] if type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel) \
        else model.model[-14]  # Detect() module
...
class DiceLoss(nn.Module):
    ...
    def f_score(self, pr, gt, beta=1, threshold=None):
        """
            Args:
        ...
        """
        if self.activation is None or activation == "none":
        ...
        score = ((1+beta **2)*tp +self.eps) /((1 + beta ** 2) * tp + beta ** 2 * fn + fp + self.eps)

        return score

    def forward(self, y_pr, y_gt):
        return 1-self.f_score(y_pr,y_gt,beta=1., threshold=None)
