...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.mask_files = [x.replace('rgb', 'mask') for x in self.img_files]
        ...
        if cache_images:  # if training
            ...
            for i in pbar:  # max 10k images
                self.imgs[i],self.masks[i], self.img_hw0[i], self.img_hw[i] = load_image(self, i)  # img, hw_original, hw_resized
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            # Load mosaic
            img,mask, labels = load_mosaic(self, index)
            shapes = None

        else:
            # Load image
            img,mask, (h0, w0), (h, w) = load_image(self, index)

            # Letterbox
            shape = self.batch_shapes[self.batch[index]] if self.rect else self.img_size  # final letterboxed shape
            img,mask, ratio, pad = letterbox(img,mask, shape, auto=False, scaleup=self.augment)
        ...
        if self.augment:
            # Augment imagespace
            if not self.mosaic:
                img, mask, labels = random_affine(img,mask, labels,
                                            degrees=hyp['degrees'],
        ...
        return torch.from_numpy(img),torch.from_numpy(mask), labels_out, self.img_files[index], shapes
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)  # BGRD
        assert img is not None, 'Image Not Found ' + path

        mask_path = self.mask_files[index]
        mask = cv2.imread(mask_path, cv2.IMREAD_UNCHANGED)  # BGRD
        assert mask is not None, 'Image Not Found ' + mask_path
        ...
        return self.imgs[index],self.masks[index], self.img_hw0[index], self.img_hw[index]  # img, hw_original, hw_resized


def augment_hsv(img, hgain=0.5, sgain=0.5, vgain=0.5):
    r = np.random.uniform(-1, 1, 3) * [hgain, sgain, vgain] + 1  # random gains
    hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGRA2HSV))
...
def load_mosaic(self, index):
    ...
    for i, index in enumerate(indices):
        # Load image
        img,mask, _, (h, w) = load_image(self, index)

        # place img in img4
        if i == 0:  # top left
            img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
            mask4 = np.full((s * 2, s * 2, mask.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
    ...
    img4,mask4, labels4 = random_affine(img4,mask4, labels4,
...
def letterbox(img,mask, new_shape=(416, 416), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True):
...
def random_affine(img,mask, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=0):
    ...
    for path in ['../data/sm4/images', '../data/sm4/background']:
        ...
        for ext in formats:  # ['.bmp', '.jpg', '.jpeg', '.png', '.tif', '.dng']
            for f in tqdm(glob.glob('%s/*%s' % (path, ext)), desc='Converting %s' % ext):
                cv2.imwrite(f.replace(ext.lower(), '.bmp').replace(path, path + 'bmp'), cv2.imread(f, cv2.IMREAD_UNCHANGED))
    ...
    for a, b, files in os.walk(dataset):
        for file in tqdm(files, desc=a):
            ...
            if s == '.txt':  # replace text
                ...
                cv2.imwrite(p.replace(s, '.bmp'), cv2.imread(p, cv2.IMREAD_UNCHANGED))
                if s != '.bmp':
                    os.system("rm '%s'" % p)
