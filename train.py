...
def train(hyp):
    ...
    if weights.endswith('.pt'):  # pytorch format
        ...
        try:
            ckpt['model'] = \
                {k: v for k, v in ckpt['model'].state_dict().items() if yolo_model.state_dict()[k].numel() == v.numel()}
    ...
    if mixed_precision:
        yolo_model, optimizer = amp.initialize(yolo_model, optimizer, opt_level='O1', verbosity=0)
    ...
    if device.type != 'cpu' and torch.cuda.device_count() > 1 and torch.distributed.is_available():
        dist.init_process_group(backend='nccl',  # distributed backend
                                ...
                                rank=0)  # node rank
        yolo_model = torch.nn.parallel.DistributedDataParallel(yolo_model)
    ...
    yolo_model.gr = 1.0  # giou loss ratio (obj_loss = 1.0 or giou)
    yolo_model.class_weights = labels_to_class_weights(dataset.labels, nc).to(device)  # attach class weights
    ...
    ema = torch_utils.ModelEMA(yolo_model)
        ...
        if save:
            with open(results_file, 'r') as f:  # create checkpoint
                ckpt = {'epoch': epoch,
                        ...
                        'model': ema.ema.module if hasattr(yolo_model, 'module') else ema.ema,
